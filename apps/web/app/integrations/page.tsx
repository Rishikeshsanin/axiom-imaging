import { BookOpenText, Boxes, Database, ExternalLink, Github, MonitorPlay, Network, ServerCog } from "lucide-react";
...
export default async function IntegrationsPage() {
  ...
  try {
    ...
    const cards = [
      ...
      { name: "C++ Device Engine", description: "Separate C++20 concurrency and telemetry simulation service over TCP IPC.", href: null, label: null, icon: ServerCog, status: health.device_engine.status },
    ];
    return <><PageHeader title="System Integrations" description="One place to inspect the services behind Axiom Imaging and open operational developer tools without manually typing localhost URLs." action={<a href={githubUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 hover:bg-slate-50"><Github size={17}/>GitHub<ExternalLink size={14}/></a>}/><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{cards.map(({ name, description, href, label, icon: Icon, status }) => <article key={name} className="flex min-h-52 flex-col rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-start justify-between gap-4"><div className="rounded-lg bg-cyan-50 p-2 text-cyan-800"><Icon size={20}/></div><StatusChip value={status}/></div><h2 className="mt-4 font-semibold text-slate-950">{name}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{description}</p>{href && label ? <a href={href} target="_blank" rel="noreferrer" className="mt-auto inline-flex items-center gap-2 pt-5 text-sm font-semibold text-cyan-800 hover:text-cyan-950">{label}<ExternalLink size={14}/></a> : <div className="mt-auto pt-5 text-xs font-medium text-slate-400">Internal service · no public console</div>}</article>)}</div><section className="mt-6 rounded-xl border border-cyan-200 bg-cyan-50 p-5 text-sm text-cyan-950"><strong>Architecture boundary:</strong> PostgreSQL stores application/workflow metadata, Orthanc stores DICOM objects and pixel data, OHIF renders DICOM through DICOMweb, and the C++ engine supplies simulated device telemetry. These are separate services by design.</section></>;
